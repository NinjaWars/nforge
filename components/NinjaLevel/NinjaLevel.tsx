...
export type ninjaLevelTier =
    | 'Novice'
    | 'Acolyte'
    | 'Ninja'
    | 'Elder Ninja'
    | 'Shadow Master'
export type ninjaLevelClasses =
    | 'novice'
    | 'acolyte'
    | 'ninja'
    | 'elderNinja'
...
const NinjaLevel = ({ className, level }: NinjaLevelProps) => {
    ...
    const tier = Object.entries(tiers).reduce((acc, [clevel, lTier]) => {
        return level >= +clevel ? lTier : acc
    }, tiers[0])
    return (
        <span
            className={`${classes.basic} ${classes[camelCase(tier) as ninjaLevelClasses]
                } ${className}`}
        >
            {tier} [{level}]
        </span>
